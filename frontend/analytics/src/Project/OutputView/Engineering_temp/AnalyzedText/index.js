...
// 각 탭에 대한 개별 화면 컴포넌트
const EntitiesScreen = ({ sentence, word1, word2, response1, response2, response3 }) => (
  <Entities sentence={sentence} word1={word1} word2={word2} />
);

const RxNormConceptsScreen = ({ response1 }) => <TumorMarkerTest response1={response1} />;

const ICD10CMConceptsScreen = ({ response2 }) => <PrimaryCancer response2={response2} />;

const SNOMEDCTConceptsScreen = ({ response3 }) => <Tumor response3={response3} />;

const AnalyzedText = ({ sentence, word1, word2, response1, response2, response3 }) => {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ padding: 2, backgroundColor: '#ffffff', minHeight: '150px' }}>
        {activeTab === 0 && <EntitiesScreen sentence={sentence} word1={word1} word2={word2} />}
