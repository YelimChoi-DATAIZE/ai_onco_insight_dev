...
const TextWithArrow = ({ sentence, word1, word2 }) => {
  ...
  const generatePathD = (positions) => {
    ...
    if (positions.topLeft < positions.topRight && positions.left < positions.right) {
      return `
        ...
        L ${positions.left} ${positions.topLeft - (30 - heightReduction)}
        Q ${positions.left} ${positions.topLeft - (50 - heightReduction)}, ${positions.left + 30} ${positions.topLeft - (50 - heightReduction)}
        L ${positions.left + containerWidth * 0.5 - 30} ${positions.topLeft - (50 - heightReduction)}
        Q ${positions.left + containerWidth * 0.5} ${positions.topLeft - (50 - heightReduction)}, ${positions.left + containerWidth * 0.5} ${positions.topLeft - (30 - heightReduction)}
        L ${positions.left + containerWidth * 0.5} ${positions.topLeft + (positions.topRight - positions.topLeft) - 35}
        Q ${positions.left + containerWidth * 0.5} ${positions.topLeft + (positions.topRight - positions.topLeft) - 15}, ${positions.left + containerWidth * 0.5 - 30} ${positions.topLeft + (positions.topRight - positions.topLeft) - 15}
  ...
  return (
    <>
      ...
      >
        {word1?.trim() && word2?.trim() && positions.left !== 0 && positions.right !== 0 && (
          <svg
      ...
      `}
        ...
        >
          {(sentence || '').split(' ').map((word, index) => {
            const isHighlighted = word === word1 || word === word2;
            ...
            return (
              <span
                ...
                style={{
                  textDecoration: isHighlighted ? 'underline 2px orange' : 'none',
                  fontWeight: isHighlighted ? 'bold' : 'normal',
                  color: isHighlighted ? 'orange' : 'black',
                }}
              >
                {word}{' '}
              </span>
